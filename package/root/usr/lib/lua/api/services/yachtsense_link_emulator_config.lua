-- Dedicated configuration writer for the YachtSense Link Emulator.
--
-- RutOS' Lua UCI binding requires list options to be written with add_list;
-- passing a Lua table to cursor:set can fail at runtime. This endpoint also
-- validates the complete form itself so JSON booleans with value false are
-- accepted normally.
...
local function valid_interface(value)
	value = trim(value)
	if not safe_interface_name(value) then
		return false
	end
...
local function write_config(data)
	...
	-- Always rebuild the UCI list with the binding's list-specific method.
...
Service:action("save", Service.SaveAction)
