-- Schema-proof YachtSense Link Emulator configuration writer for RutOS 7.
--
-- The complete form is transported as one JSON string. This deliberately avoids
-- FunctionService coercion/validation quirks for JSON booleans and arrays.
...
local function interface_has_ipv4(name)
	return #interface_addresses(name) > 0
end
...
local function validate(data)
	if type(data) ~= "table" then
		return false, "Invalid configuration payload"
	end
	...
	if not valid_interface(axiom_interface) then
	...
	local wanted_cidr = trim(data.ipaddr) .. "/" .. tostring(math.floor(tonumber(data.prefix)))
	if boolean_value(data.manage_ip) == false and not interface_has_cidr(axiom_interface, wanted_cidr) then
		return false, wanted_cidr .. " is not present on " .. axiom_interface .. "; enable Manage RayNet IPv4 address or select the interface that already owns this address"
	end
	...
	for _, name in ipairs(data.remote_interfaces) do
		name = trim(name)
		if not valid_interface(name) then
			return false, "Invalid Raymarine app interface: " .. name
		end
		if name ~= axiom_interface and not interface_has_ipv4(name) then
...
local function latest_service_error()
	local line = command_output("logread -e " .. shell_quote(LOG_TAG) .. " | tail -n 4 | tr '\\n' ' '")
	return line ~= "" and line or "service exited immediately; no service log was produced"
end

local function process_stays_running()
	os.execute("sleep 1")
...
local function write_config(data)
	...
	if values.enabled == "1" then
		...
		if not process_stays_running() then
			return false, "Configuration was saved, but the service did not stay running: " .. latest_service_error()
		end
	else
...
function Service:SaveAction()
	...
	if not success then
		return self:ResponseOK({ ok = false, message = "Saving configuration failed: " .. tostring(write_ok) })
	end
	if not write_ok then
		return self:ResponseOK({ ok = false, message = write_message })
	end
