-- YachtSense Link Emulator v1.0.10 configuration writer.
-- Uses the native UCI CLI with the custom-package config directory explicitly.
...
local function validate(data)
	...
	for _, name in ipairs(data.remote_interfaces) do
		...
		if name ~= axiom_interface and #interface_addresses(name) == 0 then return false, "Raymarine app interface " .. name .. " has no IPv4 address" end
	end
...
local function write_config(data)
	...
	if not command_ok(UCI .. " -q get " .. shell_quote(CONFIG .. "." .. SECTION)) then
		if not command_ok(UCI .. " set " .. shell_quote(CONFIG .. "." .. SECTION .. "=emulator")) then
			return false, "Could not create YachtSense configuration section in " .. UCI_DIR
		end
	end
	...
	for key, value in pairs(values) do
		if not uci_set(key, value) then return false, "Could not store configuration option " .. key .. " in " .. UCI_DIR end
	end
	...
	for _, interface in ipairs(data.remote_interfaces) do
		...
		if not seen[interface] then
			...
			if not command_ok(UCI .. " add_list " .. shell_quote(list_key .. "=" .. interface)) then
				return false, "Could not store Raymarine app interface " .. interface .. " in " .. UCI_DIR
			end
		end
	end
	if not command_ok(UCI .. " commit " .. shell_quote(CONFIG)) then return false, "Could not commit YachtSense configuration in " .. UCI_DIR end
	...
	return true, "Configuration saved and applied"
end
