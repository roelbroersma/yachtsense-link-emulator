...
--
-- The form is transported as one JSON string to avoid FunctionService coercion
-- issues. RutOS' UCI cursor exposes set_list() for list options; add_list() is
-- not a cursor method on the target firmware.
...
local function latest_service_error()
	local line = command_output("logread -e " .. shell_quote(LOG_TAG) .. " | tail -n 6 | tr '\\n' ' '")
...
local function write_config(data)
	...
	-- When address management is disabled, relinquish package ownership before
	-- restart. This prevents the init script from deleting an existing address.
