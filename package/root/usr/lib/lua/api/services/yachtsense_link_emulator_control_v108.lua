...
--
-- The target RutOS Lua `uci` binding does not expose get_list()/set_list().
-- List values are therefore read through the native uci CLI while scalar values
-- continue to use the Lua cursor. Package paths follow opkg's configured root.
...
local function read_remote_interfaces()
	local option = CONFIG .. "." .. SECTION .. ".remote_interface"
	local output = command_output("uci -q show " .. shell_quote(option))
	local result = {}
	for line in (output .. "\n"):gmatch("(.-)\n") do
		local value = line:match("='([^']+)'$") or line:match("=([^%s]+)$")
		value = trim(value)
		if value ~= "" then result[#result + 1] = value end
	end
	if #result == 0 then result[1] = "br-lan" end
	return result
end
...
local function read_config()
	...
	local config = {
		...
		remote_interfaces = read_remote_interfaces()
	}
...
local function latest_service_error()
	local line = command_output("logread -e " .. shell_quote(LOG_TAG) .. " | tail -n 8 | tr '\\n' ' '")
