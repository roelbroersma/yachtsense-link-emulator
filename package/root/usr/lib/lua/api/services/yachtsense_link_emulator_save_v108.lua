...
-- boolean/list coercion quirks. Scalar UCI values may be read through Lua, but
-- list writes deliberately use the native `uci` CLI because the target RutOS
-- Lua `uci` binding exposes neither add_list() nor set_list().
...
local function write_remote_interfaces(remotes)
	local option = CONFIG .. "." .. SECTION .. ".remote_interface"
	if not command_ok("uci -q delete " .. shell_quote(option)) then
		-- Missing list is normal; only fail when the CLI itself is unavailable.
		if not command_ok("command -v uci") then return false, "RutOS uci command is unavailable" end
	end
	for _, interface in ipairs(remotes) do
		local assignment = option .. "=" .. interface
		if not command_ok("uci add_list " .. shell_quote(assignment)) then
			return false, "Could not store Raymarine app interface " .. interface
		end
	end
	if not command_ok("uci commit " .. shell_quote(CONFIG)) then
		return false, "Could not commit Raymarine app interface list"
	end
	return true
end
...
local function write_config(data)
	...
	local list_ok, list_error = write_remote_interfaces(remotes)
	if not list_ok then return false, list_error end
