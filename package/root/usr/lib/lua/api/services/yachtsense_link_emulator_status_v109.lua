-- Stable read-only status API for YachtSense Link Emulator v1.0.9.
--
-- RutOS' Lua UCI bindings differ between firmware builds, so configuration is
-- read with the native `uci` command. All IPv4 addresses are captured with one
-- `ip -o -4 addr show` invocation per refresh, avoiding inconsistent per-device
-- samples when the WebUI polls repeatedly.
...
local function command_capture(command)
	local pipe = io.popen("(" .. command .. ") 2>/dev/null; rc=$?; printf '\n__YSLE_RC=%s\n' \"$rc\"")
	if not pipe then return "", false end
	local output = pipe:read("*a") or ""
	pipe:close()
...
local function uci_get_list(option, fallback)
	local key = CONFIG .. "." .. SECTION .. "." .. option
	local output = command_output("uci -q get " .. shell_quote(key))
	local result = {}
	...
	if #result == 0 then
		for _, value in ipairs(fallback or {}) do result[#result + 1] = value end
	end
	return result
end
...
local function interface_snapshot()
	local address_output, address_ok = command_capture("ip -o -4 addr show")
	local addresses = {}
	for line in (address_output .. "\n"):gmatch("(.-)\n") do
	...
	names:close()
	return result, address_ok, addresses
end

local function has_cidr(addresses, name, wanted)
	for _, cidr in ipairs(addresses[name] or {}) do
		if cidr == wanted then return true end
	end
	return false
end
...
local function mdns_listener_lines()
	...
	local result = {}
	for line in (output .. "\n"):gmatch("(.-)\n") do
		if trim(line) ~= "" then result[#result + 1] = trim(line) end
	end
	return result
end
...
local function collect_status()
	...
	local web_reachable = running and config.web_enabled and has_address
		and command_ok("wget -q -T 1 -O /dev/null " .. shell_quote(web_url))
	local runtime = read_runtime_state()
	local remote_present = true
	for _, name in ipairs(config.remote_interfaces) do
		if not interface_exists(name) then remote_present = false; break end
	end

	return {
		config = config,
		interfaces = interfaces,
		interfaces_snapshot_ok = snapshot_ok,
		logs = read_logs(config.log_lines),
		mdns_environment = mdns_environment(),
		status = {
			running = running,
			autostart = command_ok("ls /etc/rc.d/S*yachtsense-link-emulator >/dev/null 2>&1"),
			axiom_interface_exists = axiom_exists,
...
function Service:GET_TYPE_status()
	...
	if not success then
		return self:ResponseOK({ ok = false, error = "Status collection failed: " .. tostring(result) })
	end
