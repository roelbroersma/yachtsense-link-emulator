-- YachtSense Link Emulator v1.0.9 service controls.
--
-- Use only native RutOS commands for configuration access and wrap every action
-- so a Lua/runtime problem is returned as a normal WebUI message, never HTTP 500.
...
local function uci_get_list(option, fallback)
	...
	for value in output:gmatch("%S+") do
		if trim(value) ~= "" then result[#result + 1] = trim(value) end
	end
	if #result == 0 then
...
function Service:StopAction()
	return safe_response(self, "Stop", function()
		...
		if not set_enabled(false) then return false, "Service stopped, but configuration could not be disabled" end
