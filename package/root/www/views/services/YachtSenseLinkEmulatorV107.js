 ...
 * v1.0.7 entry point.
 *
 * Uses install-root aware save/control APIs. RutOS custom packages normally live
 * below /usr/local even when storage expansion changes the physical backing.
 */

import YachtSenseLinkEmulator from "./YachtSenseLinkEmulatorV104.js?v=1.0.7";
...
export default {
  ...
  name: "YachtSenseLinkEmulatorV107",
  ...
  methods: {
    ...
    async save() {
      ...
      try {
        ...
        const response = await this.$axios.post(
          "/api/yachtsense-link-emulator-save-v107/actions/save",
    ...
    async action(name, data = {}) {
      ...
      try {
        const response = await this.$axios.post(
          `/api/yachtsense-link-emulator-control-v107/actions/${name}`,
          { data: {} },
        );
        const result = findActionResult(response);
        if (result?.ok === false) {
