 ...
 * v1.0.8 entry point.
 *
 * Uses install-root aware service controls and native RutOS uci CLI list
 * handling, avoiding missing get_list()/set_list() methods in the Lua binding.
 */

import YachtSenseLinkEmulator from "./YachtSenseLinkEmulatorV104.js?v=1.0.8";
...
export default {
  ...
  name: "YachtSenseLinkEmulatorV108",
  ...
  methods: {
    ...
    async save() {
      ...
      try {
        ...
        const response = await this.$axios.post(
          "/api/yachtsense-link-emulator-save-v108/actions/save",
    ...
    async action(name, data = {}) {
      ...
      try {
        const response = await this.$axios.post(
          `/api/yachtsense-link-emulator-control-v108/actions/${name}`,
          { data: {} },
        );
        const result = findActionResult(response);
        if (result?.ok === false) {
