 ...
 * v1.0.6 entry point.
 *
 * Save uses the RutOS set_list based writer. Start/restart use stable-PID
 * verification and preserve an existing RayNet address when management is off.
 */

import YachtSenseLinkEmulator from "./YachtSenseLinkEmulatorV104.js?v=1.0.6";
...
export default {
  ...
  name: "YachtSenseLinkEmulatorV106",
  ...
  methods: {
    ...
    async save() {
      ...
      try {
        ...
        const response = await this.$axios.post(
          "/api/yachtsense-link-emulator-save-v106/actions/save",
    ...
    async action(name, data = {}) {
      ...
      try {
        const response = await this.$axios.post(
          `/api/yachtsense-link-emulator-control-v106/actions/${name}`,
          { data: {} },
        );
        const result = findActionResult(response);
        if (result?.ok === false) {
