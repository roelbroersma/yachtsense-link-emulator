/*
 * v1.0.10 entry point.
 * Uses package-root UCI APIs and stable single-snapshot interface status.
 */

import YachtSenseLinkEmulator from "./YachtSenseLinkEmulatorStyled.js?v=1.0.10";
...
function ensureV110Stylesheet() {
  const id = "ysle-v110-stylesheet";
  if (document.getElementById(id)) return;
  const link = document.createElement("link");
  link.id = id;
  link.rel = "stylesheet";
  link.href = "/assets/yachtsense-link-emulator-v109.css?v=1.0.10";
  document.head.appendChild(link);
}

export default {
  ...YachtSenseLinkEmulator,
  name: "YachtSenseLinkEmulatorV110",
  ...
  methods: {
    ...
    async refresh(forceForm) {
      ...
      try {
        const response = await this.$axios.get("/api/yachtsense-link-emulator-status-v110/status");
    ...
    async save() {
      ...
      try {
        ...
        const response = await this.$axios.post(
          "/api/yachtsense-link-emulator-save-v110/actions/save",
    ...
    async action(name, data = {}) {
      ...
      try {
        const response = await this.$axios.post(
          `/api/yachtsense-link-emulator-control-v110/actions/${name}`,
          { data },
        );
        const result = findActionResult(response);
